...
def supply_chain_alerts(limit=10):
    conn = get_db()
    try:
        # Simplified query - just get supply_chain data with product_name
        # Added LIMIT to prevent loading too much data
        rows = conn.execute(
            "SELECT product_id, product_name, vendor_lead_time_days, "
            "stock_level, delay_reason, region, cost_per_unit "
            "FROM supply_chain "
            "LIMIT 200"
        ).fetchall()
    finally:
